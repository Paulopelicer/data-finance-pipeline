...
from a_configs.spark_session import get_spark_session
...
REQUIRED_FILES = [
    ...
    "run_pipeline.py",
    "a_configs/config.py",
    "a_configs/spark_session.py",
    "c_ingestion/data_source.py",
    "d_processing/data_quality.py",
    "a_configs/utils.py",
    "d_processing/feature_engineering.py",
    "j_scripts/clean_outputs.py",
    "j_scripts/rebuild_pipeline.py",
    "j_scripts/validate_pipeline.py",
    "j_scripts/test_few.py",
    "j_scripts/validate_agents_skills.py",
    "j_scripts/validate_mcp_csv.py",
    "j_scripts/sync_agents_skills_to_codex.py",
    "g_data/input/README_input.md",
    "n_docs/PRD_Functional.md",
    "n_docs/PRD_Technical.md",
    "n_docs/guardrails.md",
    "n_docs/code_guardrails.md",
    "n_docs/requirements_traceability_matrix.md",
    "n_docs/agent_skills_research.md",
    "n_docs/agents_inventory.md",
    "n_docs/agent_interaction_examples.md",
    "h_mcp/README.md",
    "h_mcp/csv_mcp_server.py",
    "h_mcp/tools/csv_tools.py",
    "h_mcp/mcp_config.example.json",
    "m_reports/regression_metrics.csv",
    "m_reports/classification_metrics.csv",
    "m_reports/model_metrics_summary.csv",
    "m_reports/business_metrics_summary.csv",
]
...
REQUIRED_OUTPUTS = [
    PIX_RAW_DIR / "_SUCCESS",
    PIX_CLEAN_DIR / "_SUCCESS",
    PIX_MONTHLY_INDICATORS_DIR / "_SUCCESS",
    PIX_EDA_SUMMARY_DIR / "_SUCCESS",
    PIX_ML_FEATURES_DIR / "_SUCCESS",
    PIX_SELECTED_FEATURES_DIR / "_SUCCESS",
    PIX_REGRESSION_PREDICTIONS_DIR / "_SUCCESS",
    PIX_CLASSIFICATION_PREDICTIONS_DIR / "_SUCCESS",
    PIX_FEE_SAVINGS_DIR / "_SUCCESS",
    PIX_TRANSFER_SAVINGS_DIR / "_SUCCESS",
...
REQUIRED_GITIGNORE_PATTERNS = [
    ...
    ".pipeline_runs/",
    ".pytest_cache/",
    "m_reports/figures/*.png",
    "g_data/a_bronze/",
    "g_data/b_silver/",
    "g_data/c_gold/",
]
TEXT_PATTERNS_TO_SCAN = ["*.py", "*.md", "*.ipynb", "*.json", "*.yaml"]
EXCLUDED_PARTS = {".venv", ".git", ".pipeline_runs", "__pycache__", "g_data"}
EMOJI_RE = re.compile(
    "[" "\U0001f300-\U0001faff" "\U00002700-\U000027bf" "\U00002600-\U000026ff" "]"
)
WINDOWS_PATH_RE = re.compile(
    r"(?:[A-Za-z]:\\(?:Users|Windows|Program Files|ProgramData|Temp|tmp)|/mnt/"
    + "c/Users/)",
    re.IGNORECASE,
)
DISALLOWED_SIMULATION_RE = re.compile(
    r"dados\s+" + r"simulad[oa]s" + "|" + r"fonte\s+" + r"simulad[oa]", re.IGNORECASE
)
...
def validate_files(errors: list[str]) -> None:
    for item in REQUIRED_FILES:
        path = BASE_DIR / item
        (
            pass_msg(f"arquivo encontrado: {item}")
            if path.exists()
            else fail(f"arquivo ausente: {item}", errors)
        )
    for item in REQUIRED_NOTEBOOKS:
        path = NOTEBOOKS_DIR / item
        (
            pass_msg(f"notebook encontrado: {item}")
            if path.exists()
            else fail(f"notebook ausente: {item}", errors)
        )
    for item in REQUIRED_DIRS:
        path = BASE_DIR / item
        (
            pass_msg(f"diretorio encontrado: {item}")
            if path.exists() and path.is_dir()
            else fail(f"diretorio ausente: {item}", errors)
        )
...
def validate_gitignore(errors: list[str]) -> None:
    ...
    (
        fail(f".gitignore sem padroes obrigatorios: {missing}", errors)
        if missing
        else pass_msg(".gitignore contem caches, outputs e artefatos Spark")
    )
...
def validate_outputs(errors: list[str]) -> None:
    for path in REQUIRED_OUTPUTS:
        (
            pass_msg(f"output encontrado: {rel(path)}")
            if path.exists()
            else fail(f"output ausente: {rel(path)}", errors)
        )
    if errors:
        return
    ...
    checks = [
        ("Bronze", PIX_RAW_DIR),
        ("Silver", PIX_CLEAN_DIR),
        ("Gold indicadores", PIX_MONTHLY_INDICATORS_DIR),
        ("Gold EDA", PIX_EDA_SUMMARY_DIR),
        ("Gold features", PIX_ML_FEATURES_DIR),
        ("Gold selected features", PIX_SELECTED_FEATURES_DIR),
        ("Gold regressao", PIX_REGRESSION_PREDICTIONS_DIR),
        ("Gold classificacao", PIX_CLASSIFICATION_PREDICTIONS_DIR),
        ("Gold economia cartao", PIX_FEE_SAVINGS_DIR),
        ("Gold economia transferencia", PIX_TRANSFER_SAVINGS_DIR),
    ]
    try:
        for label, path in checks:
            count = spark.read.parquet(str(path)).count()
            (
                pass_msg(f"{label} possui {count} registros")
                if count > 0
                else fail(f"{label} esta vazio", errors)
            )
    finally:
        spark.stop()
