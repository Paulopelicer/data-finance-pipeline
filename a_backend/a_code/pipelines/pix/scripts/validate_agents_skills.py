...
SKILLS_DIR = BASE_DIR / "agents_skills"
REQUIRED_SECTIONS = ["Purpose", "Scope", "When to Use", "Inputs", "Outputs", "Required Steps", "Standards", "Constraints", "Validation", "Acceptance Criteria", "Example Prompts"]
REQUIRED_SKILLS = [
    "data-ingestion", "data-treatment", "exploratory-data-analysis", "feature-engineering", "feature-selection", "regression-modeling", "classification-modeling", "metrics-evaluation", "data-visualization", "data-quality", "documentation-prd", "run-all-pipeline", "bi-analytics", "data-engineering", "pyspark-pipeline", "pix-pipeline-generator",
]
EMOJI_RE = re.compile("[" "\U0001F300-\U0001FAFF" "\U00002700-\U000027BF" "\U00002600-\U000026FF" "]")
LOCAL_PATH_RE = re.compile(r"/home/|/mnt/" + "c/Users/" + r"|[A-Za-z]:" + r"\\", re.IGNORECASE)
...
def validate_skill(skill_name: str, errors: list[str]) -> None:
    ...
    for path in [skill_md, yaml_path, refs_dir]:
        if not path.exists():
            errors.append(f"artefato ausente em {skill_name}: {path.relative_to(skill_dir)}")
    ...
    for path in skill_dir.rglob("*"):
        if path.is_file():
            ...
            if LOCAL_PATH_RE.search(content):
                errors.append(f"caminho local encontrado em {path.relative_to(SKILLS_DIR)}")
    if skill_name != "pix-pipeline-generator" and "Pix" in text:
        errors.append(f"skill agnostica contem dependencia textual de Pix: {skill_name}")
